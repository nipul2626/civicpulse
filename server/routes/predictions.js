...
// GET /api/predictions/upcoming — rule-based pattern matching, no AI
router.get('/upcoming', verifyToken, requireRole('coordinator'), async (req, res) => {
    try {
        ...
        // For each category, compare current month count to historical average
        const predictions = VALID_CATEGORIES.map(category => {
            ...
            const confidence = totalDataPoints >= 20 ? 'high' : totalDataPoints >= 8 ? 'medium' : 'low';

            const predictedCount = Math.round(historicalAvg * 1.1); // slight buffer

            // Identify if current month is historically busy
            const isHighMonth = currentMonthCount >= historicalAvg * 1.3 || currentMonthCount === maxMonth;
        ...
        res.json({ orgId, month: now.toLocaleString('default', { month: 'long' }), predictions });
    } catch (err) {
